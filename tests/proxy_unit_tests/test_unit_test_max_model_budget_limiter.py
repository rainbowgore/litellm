...
def test_resolve_model_budget():
    ...
    resolved = resolve_model_budget(
        model="openai/gpt-4", model_max_budget=model_max_budget
    )
    assert resolved.budget_model == "gpt-4"
    assert resolved.budget_config.max_budget == 100.0
    ...
    assert (
        resolve_model_budget(model="non-existent", model_max_budget=model_max_budget)
        is None
    )
...
async def test_is_key_within_model_budget(budget_limiter):
    ...
    with patch.object(
        budget_limiter, "_get_spend_for_model_budget", return_value=150.0
    ):
...
async def test_get_spend_for_model_budget_reads_the_configured_model_key(
    ...
    with patch.object(
        budget_limiter.dual_cache, "async_get_cache", return_value=50.0
    ) as mock_get:
        ...
        assert spend == 50.0
        assert mock_get.call_args.kwargs["key"] == (
...
async def test_user_model_budget_counter_is_separate_from_the_key_counter():
    ...
    assert (
        await dual_cache.async_get_cache(key="virtual_key_spend:vk-hash:gpt-4:1d") == 2.0
    )
...
async def test_two_models_on_one_key_do_not_share_a_budget_window():
    ...
    with patch.object(
        ...
        start_time_keys = [
            call.kwargs["start_time_key"] for call in mock_increment.call_args_list
        ]
...
async def test_build_model_max_budget_usage_skips_unusable_entries():
    ...
    assert usage == {
        "gpt-4": {"current_spend": 3.0, "budget_limit": 10.0, "time_period": "1d"}
    }
...
async def test_bedrock_traffic_charges_the_bare_family_name_budget():
    ...
    with pytest.raises(litellm.BudgetExceededError):
        await limiter.is_key_within_model_budget(
            user_api_key, "bedrock/anthropic.claude-opus-4-8"
        )
...
async def test_usage_report_reads_every_counter_in_one_batched_lookup():
    ...
    with patch.object(
        dual_cache, "async_batch_get_cache", new=AsyncMock(return_value=[None] * 50)
    ) as batched, patch.object(dual_cache, "async_get_cache", new=AsyncMock()) as single:
        usage = await build_model_max_budget_usage(
            entity_type=Litellm_EntityType.KEY,
            entity_id="hash-many",
